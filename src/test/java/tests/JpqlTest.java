...
public class JpqlTest extends GenericTest {
    ...
    public void projetoPorNome() {
        logger.info("Executando projetoPorNome()");
        TypedQuery<Projeto> query = em.createQuery(
                "SELECT p FROM Projeto p WHERE p.nome LIKE :nome",
                Projeto.class);
        query.setParameter("nome", "JAVA%");
        List<Projeto> projetos = query.getResultList();

        projetos.forEach(projeto -> {
            assertTrue(projeto.getNome().startsWith("JAVA"));
        });
    ...
    @Test
    public void projetoPorNomeNamedQuery() {
        logger.info("Executando projetoPorNomeNamedQuery()");
        TypedQuery<Projeto> query = em.createNamedQuery("Projeto.PorNome", Projeto.class);
        query.setParameter("nome", "JAVA%");
        List<Projeto> projetos = query.getResultList();
...
//                Comprador.class);
//        query.setParameter(1, "VISA"); //Setando par̢metro posicional.
...
//                Comprador.class);
//        query.setParameter(1, "VISA"); //Setando par̢metro posicional.
//        query.setParameter(2, "MASTERCARD"); //Setando par̢metro posicional.        
//        List<Comprador> compradores = query.getResultList();
//
//        compradores.forEach(comprador -> {
...
//        assertEquals("MAESTRO:Jos̩ da Silva", cartoes.get(3)[0] + ":" + cartoes.get(3)[1]);
